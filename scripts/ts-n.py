...
def evaluate_model(model_name, history):
    ...
    results_file_name = f"../results/TS-N.csv"
...
def main(dataframe, train_task_name):
    ...
    X = X.replace("None", mask_value, regex=True)
    X = X.fillna(mask_value)
    X_meta = pd.DataFrame({"metadata": []})
    X_meta["metadata"] = dataframe["sensor_name"].astype(str)
    ...
    X_test = np.reshape(X_test.values, (X_test.shape[0], X_test.shape[1], 1)).astype(
        "float32"
    )
    ...
    # Masking Layer
    ts_input_layer = tf.keras.layers.Masking(mask_value=mask_value)(ts_input_layer)

    ts_encoder = time_series_encoder(ts_input_layer)
    text_encoder = text_input_layer
    ...
    model.predict([X_test, X_meta_test_embed])
...
if __name__ == "__main__":
    for _ in range(1):
        with device("/GPU:2"):
            ...
            main(dataframe, "DeepMetaIoT TS-N")
